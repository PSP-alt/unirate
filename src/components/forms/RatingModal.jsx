...
export default function RatingModal({ teacher, isOpen, onClose, onSuccess }) {
  ...
  /* Критерии: union по всем типам преподавателя (multi-type-aware) */
  ...
  const handleSubmit = async () => {
    ...
    try {
      /* Создаём оценку. Она уходит со status:'pending' — публичный
         рейтинг преподавателя НЕ пересчитывается клиентом. Это делает
         админ при approveRating (admin.js → syncTeacherStats), либо
         Cloud Function при изменении status → 'approved'. Раньше здесь
         был баг: клиент сразу инкрементировал averageRating/ratingsCount
         с учётом неодобренного отзыва, а если админ потом отклонял —
         счётчики оставались завышенными. */
      ...
      console.error('[RatingModal submit]', err)
